...
#
#  v8.6 highlights (fixes & robustness):
#    • Respects CFG window strictly (no silent override by Costs sheet)
#    • Explicit TOTAL CAP constraint when CONSERVE_TOTAL=True
#    • Coalesced prefix rows (no duplicate indices in constraints)
#    • Clearer diagnostics (whitelist/blacklist mode, horizon echo)
#    • Same lexicographic objective; same greedy warm‑start
#
#  v8.5 features retained:
#    • Robust start-time pruning (per-year & prefix vs MAX, zero-benefit culls)
#    • Greedy feasible warm-start + primary PV lower bound cut
#    • Buffer-only envelope (± levels). ±0 == exact yearly envelope
#    • Subset selection (≤1 start per project) and MAX_STARTS per FY
#    • Prefix capacity formulation (no-borrowing, carry-forward allowed)
# =====================================================================

from pathlib import Path
import pickle, time, re
from typing import Dict  # ← added for type hints only (no logic change)
import numpy as np
import pandas as pd
...
DATA_FILE = ROOT / "Scoring_latest_redacted.xlsx"

# Optional filename prefix for all saved .pkl outputs (e.g., "NCoR_").
# Set to None or "" to keep original filenames.
...
CACHE = ROOT / "scenario_cache_benefit"; CACHE.mkdir(exist_ok=True)
...
def dump_pickle(m: HighsModel, tag: str, suffix: str,
                ...
                primary_dim: str,
                year_list: list[int]):
    # NOTE: PKL_PREFIX (defined in user settings) is prepended to all filenames.
    prefix = (PKL_PREFIX or "")

    if m.SolCount == 0:
        fn = CACHE / f"{prefix}{tag}_{suffix}_noSol.pkl"
    ...
    fn = CACHE / f"{prefix}{tag}_{suffix}.pkl"
