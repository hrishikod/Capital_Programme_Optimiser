...
class CapitalProgrammeOptimizer:
    ...
    def _build_model(self):
        ...
        self.y: List[cp_model.IntVar] = [self.model.NewBoolVar(f"y_{t}") for t in range(self.years)]
        ...
        for variant_id in self.variants:
            if self.allowed_starts[variant_id]:
                self.model.Add(sum(self.x[(variant_id, s)] for s in self.allowed_starts[variant_id]) == 1)
        ...
        for t in range(1, self.years):
            ...
            self.model.Add(self.net[t] + self.spend_vars[t] + self.dividend[t] == self.net[t - 1] + self.funding[t])
