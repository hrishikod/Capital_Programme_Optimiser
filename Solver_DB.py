# =====================================================================
#  NZTA front‑load optimiser – benefit‑secure, dimension‑aware (v8.6, HiGHS)
#
#  Databricks-optimised wrapper (keeps core logic unchanged):
#    • Auto paths for DBFS vs local
#    • Parallel orchestration across scenarios/cost-types (Spark or Processes)
#    • HiGHS threads tuned to available cores
#    • No GPU dependencies; remains CPU-bound (HiGHS)
# =====================================================================
...
import numpy as np
import pandas as pd
import highspy as hs  # pip install highspy

# Try Spark if present (Databricks); safe to import when local
try:
...
def _in_databricks() -> bool:
    # Databricks sets this environment variable
    return "DATABRICKS_RUNTIME_VERSION" in os.environ

def _default_root() -> Path:
    if _in_databricks():
        # Writeable & convenient location in DBFS
        return Path(os.environ.get("NZTA_ROOT", "/dbfs/FileStore/nzta/Project_Rons_optimisation"))
    # Local Windows default (your original)
    return Path(os.environ.get("NZTA_ROOT", r"C:\Users\Adrian Desilvestro\Documents\NZTA\Project_Rons_optimisation"))

def _detect_threads() -> int:
    # Prefer Databricks-provided core counts when available
    for k in ("DATABRICKS_TASK_WORKER_CORES", "DATABRICKS_NUM_WORKER_CORES",
              "SPARK_WORKER_CORES", "NUM_CORES", "OMP_NUM_THREADS"):
        v = os.environ.get(k)
        try:
            if v and int(v) > 0:
                return int(v)
...
def _log_platform_banner():
    where = "Databricks" if _in_databricks() else "Local"
    threads = _detect_threads()
    gpu_env = os.environ.get("NVIDIA_VISIBLE_DEVICES", "")
    gpu_msg = " (GPU visible)" if gpu_env not in ("", "void", "none", None) else ""
    print(f"[platform] Running on {where}{gpu_msg}; HiGHS threads={threads}")

# ═════════════════════════════════════════════════════════════════════
#  TOGGLES
# ═════════════════════════════════════════════════════════════════════
RUN_BENEFIT_PLUSMINUS = True  # buffer logic only; ±0 emulates fixed

# ---------- user settings ---------------------------------------------
ROOT = _default_root()
DATA_FILE = Path(os.environ.get("NZTA_DATA_FILE", str(ROOT / "Scoring_latest_redacted.xlsx")))

# Cost-types to run (Costs sheet contains all types)
COST_TYPES_RUN = ["P50 - Real", "P95 - Real"]

# BENEFIT SCENARIOS -> sheet names (precomputed annual benefit tables)
BENEFIT_SCENARIOS = {
    "A60": "Benefits-A60",
    "A45": "Benefits-A45",
    "B60": "Benefits-B60",
...
# Objective dimensions to run (if empty -> auto from Benefits sheets incl. Total)
PRIMARY_OBJECTIVE_DIMS_TO_RUN: List[str] = []

# ☑️  per‑project rules (include/exclude + optional forced start)
...
def _late_weights(ny: int) -> List[float]:
    idx = np.arange(1, ny + 1, dtype=float)
    w = idx / ny
...
CFG = {"YEARS": 50, "START_FY": 2025,
       "MAX_STARTS": 15,
       "CASH_PV_RATE": 0.015}  # reporting only (PV of stocks)

EARLY_STOP_ON  = True
EARLY_STOP_PCT = .01

CACHE = Path(ROOT) / "scenario_cache_benefit"
...
def _keep_and_forced_from_rules(rules: dict, variants: dict) -> Tuple[set, Dict[str, int]]:
...
def load_costs_and_calendar(cost_type: str):
    ...
    """
    # DBFS is a FUSE mount; pandas can read directly
...
class HighsModel:
    ...
    def _apply_options(self):
        ...
        # Tune threading for Databricks / local cores
        self.h.setOptionValue("threads", int(os.environ.get("HIGHS_THREADS", _detect_threads())))
        # Keep presolve on (safe speed-up)
...
# ---------- pickle writer ---------------------------------------------
def dump_pickle(m: HighsModel, tag: str, suffix: str,
                baseline_surplusM: float,
                proj: dict, var: dict,
...
def optimise_family_for(ct: str, scenario_key: str, scenario_sheet: str):
    ...
    for primary_dim in dims_to_run:
...
def _combos_to_run() -> List[Tuple[str, str, str]]:
    return [(ct, key, sheet) for ct in COST_TYPES_RUN for key, sheet in BENEFIT_SCENARIOS.items()]

def _run_combo_picklable(args: Tuple[str, str, str]) -> None:
    # top-level function so Spark workers / process pool can pickle it
    ct, key, sheet = args
    print(f"\n=== {ct} ===\n>> Benefit Scenario: {key} (sheet: {sheet})")
    optimise_family_for(ct, key, sheet)

def _run_parallel():
    combos = _combos_to_run()
    if _in_databricks() and _spark is not None:
        # Cluster-scale parallel: each combo on an executor
        sc = _spark.sparkContext
        print(f"[parallel] Using Spark across {len(combos)} combos")
        sc.parallelize(combos, len(combos)).foreach(_run_combo_picklable)
    else:
        # Driver-scale parallel (local / jobs without Spark)
        from concurrent.futures import ProcessPoolExecutor, as_completed
        workers = _detect_threads()
        print(f"[parallel] Using ProcessPool with {workers} workers across {len(combos)} combos")
        with ProcessPoolExecutor(max_workers=workers) as ex:
            futs = [ex.submit(_run_combo_picklable, c) for c in combos]
...
def main():
    ...
    # Run all (parallel where available)
    _run_parallel()
